...
export function calcScore(pos, stats) {
  ...
  if (pos === 'DEF') return cs * 3 + buts + passD + note + resultBonus + mvp;
  return 0;
}
...
/**
 * Calcule, pour chaque paire de joueurs ayant déjà joué dans la même équipe,
 * le nombre de matchs joués ensemble et leur taux de victoire commun.
 * `matches` doit inclure match_players(*, players(name,pos)) et score_a/score_b/score_c.
 */
export function getBestDuos(matches, minTogether = 2) {
  const pairStats = {}; // key: "id1|id2" (triés) -> { ids, names, poss, together, wins, nuls }

  const teamScore = (m, t) => (t === 'A' ? m.score_a : t === 'B' ? m.score_b : m.score_c) ?? 0;

  (matches || []).forEach(m => {
    const mp = m.match_players || [];
    ...
    const scores = teams.map(t => teamScore(m, t));
    const maxScore = Math.max(...scores);
    const winners = teams.filter(t => teamScore(m, t) === maxScore);
    const isDraw = winners.length > 1;

    teams.forEach(t => {
      const teamPlayers = byTeam[t];
      const won = !isDraw && winners[0] === t;
      for (let i = 0; i < teamPlayers.length; i++) {
        for (let j = i + 1; j < teamPlayers.length; j++) {
          const a = teamPlayers[i], b = teamPlayers[j];
          if (!a.player_id || !b.player_id) continue;
          const key = [a.player_id, b.player_id].sort().join('|');
          if (!pairStats[key]) {
            pairStats[key] = {
              ids: [a.player_id, b.player_id],
              names: [a.players?.name || '?', b.players?.name || '?'],
              poss: [a.players?.pos || 'MIL', b.players?.pos || 'MIL'],
              together: 0, wins: 0, nuls: 0,
            };
          }
          pairStats[key].together++;
          if (won) pairStats[key].wins++;
          if (isDraw && winners.includes(t)) pairStats[key].nuls++;
        }
      }
    });
  });
  ...
  return Object.values(pairStats)
    .filter(d => d.together >= minTogether)
    .map(d => ({
      ...d,
      // Système de points façon championnat : victoire = 3, nul = 1, défaite = 0.
      // winRate = points obtenus / points max possibles (3 par match).
      winRate: Math.round(((d.wins * 3 + d.nuls * 1) / (d.together * 3)) * 100),
    }))
    .sort((a, b) => b.winRate - a.winRate || b.together - a.together);
}

export const DEFAULT_FORMATION = { DEF: 3, MIL: 2, ATQ: 1 };

/**
 * Sélectionne les 6 meilleurs joueurs d'une période (semaine ou saison) selon
 * une formation donnée (par défaut 2 DEF / 2 MIL / 2 ATQ pour du 6v6).
 * `playersScored` : [{ id, name, pos, score }] — un score déjà calculé
 * (score du jour pour une semaine, avgScore pour la saison).
 * Si une position n'a pas assez de joueurs, les slots restants sont
 * complétés par les meilleurs scores toutes positions confondues.
 */
export function pickTeamOfPeriod(playersScored, formation = DEFAULT_FORMATION) {
  const byPos = { ATQ: [], MIL: [], DEF: [] };
  playersScored.forEach(p => { if (byPos[p.pos]) byPos[p.pos].push(p); });
  ...
  if (team.length < totalNeeded) {
    const usedIds = new Set(team.map(p => p.id));
    const rest = playersScored.filter(p => !usedIds.has(p.id)).sort((a, b) => b.score - a.score);
    team.push(...rest.slice(0, totalNeeded - team.length));
  }

  return team.sort((a, b) => b.score - a.score);
}

/**
 * Emoji + label fun pour un duo, basés sur son taux (moyenne en %),
 * jamais sur un chiffre cumulé (nb de matchs, victoires brutes…).
 */
export function getDuoVibe(winRate) {
  if (winRate >= 90) return { emoji: '🐐', label: 'Duo légendaire' };
  if (winRate >= 75) return { emoji: '🔥', label: 'Duo en feu' };
  if (winRate >= 60) return { emoji: '⚡', label: 'Duo qui claque' };
  if (winRate >= 50) return { emoji: '🤝', label: 'Bon duo' };
  if (winRate >= 40) return { emoji: '😐', label: 'Duo moyen' };
  if (winRate >= 25) return { emoji: '😬', label: 'Duo fragile' };
  if (winRate >= 10) return { emoji: '💀', label: 'Duo maudit' };
  return { emoji: '🚨', label: 'Séparez-les !' };
}
...
export const RATING_SCALE = {
  ATQ: [
    ...
    { label: 'Décisif',          icon: '⚡', pts: 2 },
    { label: 'Appels de balle',  icon: '📡', pts: 2 },
    { label: 'Retour défensif',  icon: '📍', pts: 2 },
